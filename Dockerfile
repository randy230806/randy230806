...
RUN echo 'randyZ' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/randyZ.sh

RUN echo 'cd ' >>/randyZ.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/randyZ.sh

RUN echo 'cd /noVNC-1.2.0' >>/randyZ.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/randyZ.sh

RUN chmod 755 /randyZ.sh
